class Advertisement < ApplicationRecord
  ...
  scope :published_and_sell, -> {
    where(status: 'published', ad_type: 'sell')
  }

  scope :published_and_buy, -> {
    where(status: 'published', ad_type: 'buy')
  }
  ...
  scope :active_user, -> {
    joins(:user).where("users.status = 0")
  }

  scope :user_sell_ads, ->(user) {
    joins(:user).where(user: user, ad_type: 'sell')
  }

  scope :user_buy_ads, ->(user) {
    joins(:user).where(user: user, ad_type: 'buy')
  }

  scope :user_published_sell_ads, ->(user) {
    joins(:user).where(user: user, ad_type: 'sell', status: 'published')
  }
  
  scope :user_published_buy_ads, ->(user) {
    joins(:user).where(user: user, ad_type: 'buy', status: 'published')
  }
